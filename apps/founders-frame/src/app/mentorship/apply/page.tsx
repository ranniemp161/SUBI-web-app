        ...
        <FadeIn>
          <div className="text-center max-w-2xl mx-auto mb-10">
            ...
            <p className="text-gray-400 text-base md:text-lg">
              Book your 1-on-1 strategy call directly on the calendar below or submit your founder details.
            </p>

            {/* TAB SELECTOR */}
            <div className="inline-flex items-center gap-2 p-1.5 rounded-full glass-panel border border-white/10 mt-6">
              <button
                type="button"
                onClick={() => setActiveTab("calendly")}
                className={`flex items-center gap-2 px-5 py-2 rounded-full text-xs font-bold transition-all cursor-pointer ${
                  activeTab === "calendly"
                    ? "bg-brand text-black shadow-[0_0_15px_rgba(255,255,0,0.3)]"
                    : "text-gray-300 hover:text-white"
                }`}
              >
                <Calendar className="w-3.5 h-3.5" /> Book Calendar Directly
              </button>
              <button
                type="button"
              ...
              >
                <Sparkles className="w-3.5 h-3.5" /> Qualification Form
              </button>
            </div>
          </div>
        </FadeIn>
        ...
        {activeTab === "calendly" ? (
          <FadeIn delay={0.1}>
            <div className="space-y-6">
              {/* CALENDLY EMBEDDED INLINE WIDGET */}
              <CalendlyWidget url="https://calendly.com/tunjibamgbola/30min" height="720px" />

              <div className="flex flex-wrap items-center justify-between gap-4 text-xs text-gray-400 glass-panel p-4 rounded-2xl border border-white/10">
                <div className="flex items-center gap-2">
                  <ShieldCheck className="w-4 h-4 text-brand" />
                  <span>30-minute 1-on-1 strategy session with Tunji Bamgbola</span>
                </div>
