 ...
 *
 * **A project starts one of two ways** (spec `broll/0007` AC-122): with a
 * character the creator already owns, which is free and skips the photo and the
 * review entirely, or with a style, which is the old path and leaves the
 * character to be generated on the project page. The two are exclusive, so
 * picking a character replaces the style choice rather than sitting beside it —
 * the six images exist in exactly one style and nothing may claim otherwise.
 */
...
export function NewProjectForm({
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      // The style travels either way and the server ignores it when a character
      // is named, because the character's own style is the one that is true.
      ...
      <form onSubmit={onSubmit} className="mt-6 grid gap-6">
        ...
        {characters.length > 0 && (
          <div className="grid gap-3">
            ...
            <p className="text-xs" style={{ color: "var(--broll-muted)" }}>
              Use one you already made, free, or start a new one for {setPrice}
              {" "}on the next screen.
            </p>
            ...
            >
              <button
                ...
                className="broll-glass rounded-lg p-3 text-left text-sm transition-colors"
                style={{
                  border:
                    characterId === null
              ...
              >
                <span className="flex aspect-3/4 items-center justify-center rounded text-xs"
                  style={{
                    background: "var(--broll-surface-alt)",
                    color: "var(--broll-muted)",
                  }}
                >
                  Photo, later
                </span>
                <span className="mt-2 block font-medium">New character</span>
                <span className="text-xs" style={{ color: "var(--broll-muted)" }}>
                  {setPrice}
                </span>
              </button>
        ...
        {picked ? (
          <p className="text-sm" style={{ color: "var(--broll-muted)" }}>
            This project will use <strong>{picked.name}</strong>, so its style is{" "}
