...
| `@repo/server-shared/ip-rate-limit` (not in this app) | Per-IP limiter for the routes in `proxy.ts`'s public list (no session to key on). Was `src/lib/ip-rate-limit.ts`; both apps had a byte-identical copy, so it moved to the package |
...
| `src/lib/transcript-limits.ts` | `MAX_PATCH_TRANSCRIPT_WORDS` / `MAX_PATCH_TRANSCRIPT_TEXT_CHARS` — pure and zero-dependency for the same reason as `blob-path.ts`: `validation.ts` builds the schema from them and the studio page checks against them, without pulling zod into the browser bundle |
...
| `@repo/server-shared/authz`, `@repo/server-shared/users` (not in this app) | Write-route authorization — the `users` row (provisioned by the Clerk webhook or its fallback) IS the authorization; there is no separate access-code verify route (`src/lib/access-codes.ts` no longer exists). Were `src/lib/authz.ts` and `src/lib/users.ts`; both apps gate on the same row, so they moved to the package and this app's (safer) implementation is the one that survived. See `packages/server-shared/AGENTS.md` |
...
- **The project list is keyset-paged, and the cursor must carry microseconds.** `listProjectPage` (`lib/projects.ts`) is the single query behind both the dashboard's `loadMoreProjects` server action and `GET /api/projects`. Its cursor is `<created_at>|<id>`, where the timestamp is formatted **in SQL** (`to_char(... 'US')`) rather than from the returned value: `created_at` comes back through the driver as a JS `Date`, which is millisecond-precision, while Postgres stores microseconds. The old cursor was built from that `Date`, so `created_at < cursor` also excluded rows created in the *same millisecond but microseconds earlier* — they were silently dropped from every page. The key is the pair `(created_at, id)` because timestamps aren't unique, and the `ORDER BY` must match the comparison or the keyset is unsound. A cursor that doesn't parse is an **error**, never a silent restart at page one — a "load more" that re-serves page one loops forever.
- **`transcriptSchema` must stay field-for-field with `Transcript` in `lib/edl.ts`, because a zod `object` silently strips what it doesn't declare.** `utteranceEnds` was missing from the schema, so the word-alignment PATCH — which round-trips the *entire* transcript through it — dropped Deepgram's utterance boundaries on the way to the database. Nothing errored; retake detection and the subtitle/transcript export just quietly fell back to punctuation-only sentence grouping on any project that had run the pass. If you add a field to `Transcript`, add it here too, and add a case to `validation.test.ts`.
- **A client may only PATCH a transcript up to `MAX_PATCH_TRANSCRIPT_WORDS` (40,000, ~3.4 MB).** Vercel's serverless request body cap is ~4.5 MB and a stored word serializes to ~89 bytes, so past that the platform answers a bare 413 before any of our code runs. This binds *only* the client PATCH — the transcription routes write server-side and never cross a request body, so a longer recording still transcribes, edits, and exports normally; it just can't persist its refined word boundaries. `runWordAlignment` checks the same constant first so it doesn't spend a doomed round trip, and leaves `wordsAligned` false, which is accurate: the stored copy is unrefined.
