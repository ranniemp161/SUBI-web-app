 ...
 * session metadata (userId, creditSeconds) was written server-side at session
...
export async function POST(request: Request) {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    const creditSeconds = Number(session.metadata?.creditSeconds);

    if (!userId || !Number.isInteger(creditSeconds) || creditSeconds <= 0) {
      reportError(
    ...
    try {
      const deposited = await depositPurchase(userId, creditSeconds, session.id);
      if (!deposited) {
