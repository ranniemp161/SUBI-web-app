/**
 * Timecode and timebase helpers for the NLE export formats (FCPXML, CMX 3600
 * EDL, FCP7 XML) and for B-Roll's timecode-named clips. The core seconds/ms to
 * frame rounding lives in `frame-math.ts` so every path shares one rounding
 * rule (spec 0004); `toFrames` here delegates to it. This file keeps the
 * timecode-specific parts: the fallback rate, standard-rate snapping,
 * drop-frame counting, and SMPTE timecode formatting.
 *
 * The timebase is the source clip's detected frame rate (measured by
 * rough-cut's `src/lib/detect-frame-rate.ts`, persisted on
 * `projects.source_fps_num` / `source_fps_den`). `DEFAULT_FPS` is the
 * last-resort rate only; as of spec 0004 the export action is disabled until
 * the real source rate is known, and spec _root/0001 keeps that status on the
 * transcript export too — `DEFAULT_FPS` is never silently substituted for a
 * missing rate on any real export path, it stays only as a definitional
 * default for these helpers and their tests.
 */
import { secondsToFrame, type VideoFps } from "./frame-math";

// Re-exported so existing importers of `VideoFps` from this module keep working;
// its canonical home is now `frame-math.ts`.
export type { VideoFps } from "./frame-math";
