...
/**
 * There is deliberately **no zero-money claim pair here any more.**
 *
 * `claimBrollGeneration` / `releaseBrollClaim` took `broll_projects.gen_claim_at`
 * with a zero `hold_micros` so a free regeneration could be the only writer on a
 * project. Spec `broll/0007` made a character shareable between projects, and a
 * project scoped claim stops serializing the moment two projects write the same
 * six rows. The claim moved to the row it protects, `broll_characters`, and it
 * moves no money at all, so it lives beside those rows in
 * `apps/broll/src/lib/characters.ts` rather than here. What stays in this file is
 * what it always was: every statement that touches a balance.
 */
