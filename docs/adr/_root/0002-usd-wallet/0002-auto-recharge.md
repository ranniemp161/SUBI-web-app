...
2. **[x] Save-card path (AC-5).** Checkout route now attaches/reuses a Stripe Customer and sets `setup_future_usage: off_session`; `POST /api/billing/setup-intent` starts the settings add/replace-card flow. The webhook (`checkout.session.completed` + `setup_intent.succeeded`) persists `stripe_customer_id` and `default_payment_method_id`. **Fix (2026-07-08, `/develop auto-recharge`):** the settings-path client confirm was never actually built — the "Add card" button called `setup-intent` and just showed a success message with no card form, so no card could ever be saved that way. Added `apps/wallet/src/lib/stripe-client.ts` (Stripe.js singleton) and `apps/wallet/src/app/dashboard/add-card-form.tsx` (Stripe Elements `PaymentElement`, confirms the SetupIntent via `stripe.confirmSetup`), wired into `autorecharge-panel.tsx`. Installed `@stripe/stripe-js` + `@stripe/react-stripe-js`, added `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY`. Typecheck/lint clean; not yet click-tested in a real browser session (needs `/verify`).
...
4. **[x] Cron sweep (AC-6).** `GET /api/cron/autorecharge` (Bearer `CRON_SECRET`, registered in root `vercel.json`) selects eligible users and charges off-session with the idempotency key + daily-cap check. **Cadence changed 2026-07-11**: built and originally scheduled every 2 minutes, but Vercel's **Hobby plan caps cron jobs at once per day**, so the client-preview deploy runs `0 5 * * *` instead. This is a real, intentional degradation of the feature's contract (a user who drops below threshold just after the sweep can wait up to ~23h before auto-recharge fires, during which the child `0001` zero-balance CHECK gates their service). See Follow-up below.
...
- **Cron cadence is platform-limited on Hobby, not a design choice.** Vercel's Hobby plan only allows daily cron invocations; a Pro plan (or any plan without the once-a-day cap) is required to restore the intended near-real-time sweep (every 1 to 5 minutes). Revisit `vercel.json`'s `/api/cron/autorecharge` schedule when the project moves off Hobby, and tighten it back down. Until then, the ~23h worst-case exposure window is an accepted, documented risk for the client-preview deploy, not a bug.
